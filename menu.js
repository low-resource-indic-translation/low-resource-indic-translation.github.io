document.write('\
  [<A HREF="http://www2.statmt.org/wmt23/index.html">HOME</A>] \
  <br> \
  TRANSLATION TASKS: \
  [<A HREF="http://www2.statmt.org/wmt23/translation-task.html">GENERAL MT (NEWS)</A>] \
  [<A HREF="https://wmt-terminology-task.github.io/">TERMINOLOGY</A>] \
  [<A HREF="http://www2.statmt.org/wmt23/word-autocompletion.html">WORD-LEVEL AUTOCOMPLETION</A>] \
  <br> \
');

/*
  [<A HREF="/wmt23/index.html">HOME</A>] \
  [<A HREF="/wmt23/program.html">SCHEDULE</A>] \
  <br> \
  TRANSLATION TASKS: \
  [<A HREF="/wmt23/translation-task.html">GENERAL MT (NEWS)</A>] \
  [<A HREF="/wmt23/biomedical-translation-task.html">BIOMEDICAL</A>] \
  [<A HREF="/wmt23/large-scale-multilingual-translation-task.html">LARGE-SCALE MULTILINGUAL AFRICAN</A>] \
  [<A HREF="/wmt23/efficiency-task.html">EFFICIENCY</A>] \
  [<A HREF="https://www.wmt-slt.com/">SIGN LANGUAGE</A>] \
  [<A HREF="/wmt23/code-mixed-translation-task.html">CODE MIXED</A>] \
  [<A HREF="https://wmt-chat-task.github.io/">CHAT</A>] \
  [<A HREF="/wmt23/unsup_and_very_low_res.html">UNSUP AND VERY LOW RES</A>] \
  <br> \
  EVALUATION TASKS: \
  [<A HREF="https://wmt-metrics-task.github.io/">METRICS</A>] \
  [<A HREF="https://wmt-qe-task.github.io/">QUALITY ESTIMATION</A>] \
  <br> \
  OTHER TASKS: \
  [<A HREF="/wmt23/word-autocompletion.html">WORD-LEVEL AUTOCOMPLETION</A>] \
  [<A HREF="/wmt23/translation-suggestion-task.html">TRANSLATION SUGGESTION</A>] \
  [<A HREF="/wmt23/ape-task.html">AUTOMATIC POST-EDITING</A>] \
');
*/
